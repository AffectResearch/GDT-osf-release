...
def plot_affectvsoutcome_dice(all_data, mode=None, seed=None, ylabel="Outcome Value", target_val=6, num_throws=10):
    ...
    # --- ALIGNMENT LOGIC ---
    # Since Affect = Value - Target (6), shift the limits by exactly the target_val
...
def plot_affectvsoutcome_corridor(all_data, mode="gradual", ylabel="Outcome Value", length=6, target_val=6):
    ...
    y1_min, y1_max = -4, 10 
    # We define the Y2 range (Affect) to align: Affect = Position - 6
...
# # --- EXPERIMENT 2: CORRIDOR POLICY COMPARISON ---
...
for belief in belief_modes:
    ...
    for i, s in enumerate(seeds):
        # Pass the belief mode to task runner
