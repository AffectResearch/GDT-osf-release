...
class AffectModel:
    ...
    def discrepancy (self, state, goal):
        ...
        If a custom discrepancy_fn is provided, use that;
        otherwise fall back to scalar/tuple absolute difference.
        """
        if self.discrepancy_fn is not None and self.env is not None:
            d = self.discrepancy_fn(state, goal, self.env)
        else:
...
class DecisionMaking:
    ...
    def decide(self, actions, state, goal, prev_d):
        ...
        if d > 0:
            ...
            # choose a "neutral" action if available (0), else just the first action
            action = 0 if (hasattr(actions, "__contains__") and 0 in actions) else actions[0]
...
class Agent:
    def __init__(self, env, goal, planning=False, discrepancy_fn=None):
        ...
        self.memory = Memory()
        self.metrics = []  # time-series rows for plotting
        ...
        self.affect_model.discrepancy_fn = discrepancy_fn
